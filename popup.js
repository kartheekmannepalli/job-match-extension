...
// ── HTML escaping ─────────────────────────────────────────────────────────────
// Everything rendered below flows from arbitrary web pages (page titles/URLs)
// and from the Claude response (which echoes job-description text). All of it is
// injected via innerHTML, so it MUST be escaped to avoid broken markup and
// HTML-injection. The MV3 popup CSP blocks inline <script>/event-handler
// execution, but escaping is still required for correctness and defense-in-depth.
function esc(value) {
...
function buildCategory(cat, idx) {
  ...
  const matched = (cat.matched || []).map(m => `<span class="tag matched">${esc(m)}</span>`).join('');
  const gaps = (cat.gaps || []).map(g => `<span class="tag gap">${esc(g)}</span>`).join('');
  return `
    <div class="category" id="cat-${idx}">
      <div class="category-header" data-action="toggleCat" data-arg="${idx}">
        ...
        <span class="category-score ${scoreClass(cat.score)}">${esc(cat.score)}%</span>
      ...
      <div class="category-detail" id="detail-${idx}">
        ...
        ${cat.insight ? `<div class="insight">${esc(cat.insight)}</div>` : ''}
      </div>
    </div>`;
}
...
function buildKeywords(found = [], missingMustHave = [], missingNiceToHave = []) {
  ...
  const foundTags = found.map(k => `<span class="tag matched">${esc(k)}</span>`).join('');
  const mustHaveTags = missingMustHave.map(k => `<span class="tag must-have-gap">${esc(k)}</span>`).join('');
  const niceToHaveTags = missingNiceToHave.map(k => `<span class="tag nice-to-have-gap">${esc(k)}</span>`).join('');
  return `
  ...
  const checks = (ats.checks || []).map(c => `
    <div class="ats-check">
      ...
      <div style="flex:1;min-width:0">
        ...
        ${c.detail ? `<div class="ats-check-detail">${esc(c.detail)}</div>` : ''}
      </div>
    </div>`).join('');
  const tips = (ats.tips || []).map(t => `<li>${esc(t)}</li>`).join('');
  return `
    <div class="ats-section">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin:7px 0 9px">
        ${ats.verdict ? `<span class="ats-verdict" style="color:${color};background:${color}1f;border:1px solid ${color}55">${esc(ats.verdict)}</span>` : ''}
        ${ats.requiredCoverage ? `<span style="font-size:11px;color:var(--muted)">${esc(ats.requiredCoverage)}</span>` : ''}
      </div>
  ...
  return `
    <div class="cover-letter-section">
      ...
      <div class="cover-letter-box">
        ...
        <div class="cover-letter-text" id="coverLetterText">${esc(text)}</div>
...
async function renderHistory() {
  ...
  const items = history.map((h, i) => {
    ...
    const scoreCell = noSponsor
      ...
      : `<div class="history-score" style="color:${scoreColor(h.analysis?.overallMatch || 0)}">${esc(h.analysis?.overallMatch || 0)}%</div>`;
    const subline = noSponsor
      ? `<span style="color:#f87171">No visa sponsorship</span> · ${timeAgo(h.timestamp)}`
      : `${esc(role.company || '')} · ${timeAgo(h.timestamp)}`;
    return `
      <div class="history-item" data-action="loadHistoryItem" data-arg="${i}">
        ${scoreCell}
        <div class="history-info">
          <div class="history-title">${esc(role.title || h.pageTitle || 'Job Analysis')}</div>
  ...
  root.innerHTML = `
    <div class="results">
      <div class="role-header">
        ...
        <div class="role-company">${esc(role.company || 'Company')}</div>
        <div class="role-title">${esc(role.title || 'Job Title')}</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:7px;align-items:center">
          ${role.level ? `<span class="role-level">${esc(role.level)}</span>` : ''}
      ...
      ${analysis.applicationAdvice ? `
        ...
        <div class="advice-section">
          ...
          <div class="advice-text">${esc(analysis.applicationAdvice)}</div>
        </div>` : ''}
  ...
  root.innerHTML = `
    <div style="padding:28px 20px 24px;text-align:center;">
      ...
      <p style="font-size:12px;color:var(--muted);line-height:1.6;margin-bottom:18px;">
        <strong style="color:var(--text);">${esc(role.title || 'This role')}</strong>
        ${role.company ? `at <strong style="color:var(--text);">${esc(role.company)}</strong>` : ''}
...
// In Manifest V3 the background service worker can go idle/evicted mid-request,
// in which case chrome.runtime.sendMessage may never resolve and the spinner
// would spin forever. Race it against a timeout so we always surface a result.
function sendMessageWithTimeout(message, ms = 100000) {
  let timer;
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => reject(new Error('TIMEOUT')), ms);
  });
  // clearTimeout on settle so the timer doesn't linger and reject after the
  // race already resolved (which would surface as an unhandled rejection).
...
async function startAnalysis() {
  ...
  try {
    ...
    renderError(err.message === 'TIMEOUT'
      ? 'Analysis timed out. The background service may have gone idle — please click Try Again.'
    ...
    return `
      <div class="tab-item${noSponsor ? ' tab-item-blocked' : ''}">
        <input type="checkbox" class="tab-check" id="check-${i}"
          data-tab-id="${t.id}"
          data-tab-url="${esc(t.url || '')}"
          data-tab-title="${esc(t.title || '')}">
        <label for="check-${i}" class="tab-label">
          <div class="tab-title">${esc(t.title || domain)}</div>
          <div class="tab-domain">${esc(domain)}${statusLine}</div>
        </label>
        ${rightBadge}
      </div>`;
  }).join('');
...
async function analyzeOneTab(tab) {
  ...
  try {
    ...
    const msg = err.message === 'TIMEOUT' ? 'Timed out' : (err.message || 'Analysis failed');
  ...
  const items = tabs.map(t => `
    <div class="progress-item" id="prog-${t.id}">
      ...
      <div class="progress-info">
        <div class="progress-title">${esc((t.title || t.url || '').substring(0, 60))}</div>
...
window.renderMultiResults = function (successful, blocked, errors) {
  const successItems = (successful || []).map((o, i) => {
    ...
    return `
      <div class="multi-result-item" data-action="viewMultiResult" data-arg="${i}">
        <div class="multi-score" style="color:${scoreColor(score)}">${esc(score)}%</div>
        <div class="multi-info">
          <div class="multi-title">${esc(role.title || o.tab.title || 'Job')}</div>
          <div class="multi-company">${esc(role.company || '')}${role.level ? ' · ' + esc(role.level) : ''}</div>
        </div>
  ...
  const blockedItems = (blocked || []).map(o => `
    <div class="multi-result-item multi-result-blocked">
      ...
      <div class="multi-info">
        <div class="multi-title">${esc(o.role?.title || o.tab?.title || 'Job')}</div>
  ...
  const errorItems = (errors || []).map(o => `
    <div class="multi-result-item multi-result-error">
      <div class="multi-score">❌</div>
      <div class="multi-info">
        <div class="multi-title">${esc(o.tab?.title || 'Job')}</div>
        <div class="multi-company" style="color:#f87171">${esc(o.error || 'Failed')}</div>
      </div>
    </div>`).join('');
...
async function init() {
  try {
    ...
    // Never leave the popup blank — fall back to the idle screen.
    console.error('[JobMatch] init failed, falling back to idle:', err);
    try { renderIdle(); }
    catch (_) {
      root.innerHTML = `
        <div class="state">
          <div class="state-icon">⚠️</div>
          <h2>Couldn't load</h2>
          <p>Please close and reopen the extension.</p>
        </div>`;
    }
  }
}
